...
typedef struct { // this probably belongs into str.h
    char *start;
    char *end;
} substring_t;

typedef struct { // this probably needs to be a tempalte, but it certainly shouldnt be here
    size_t entry_count;
    char **key;
    char **value;
} dictionary_t;

typedef struct { // same as above, maybe a storage and sorting header
	bool success;
	size_t index;
...
char *format(char *fmt, dictionary_t dictionary); // idk
...
char *stringAfter(char *input, size_t index);
...
// this is funny and needs to go into some global internal header
...
// the right place would be in str.h
